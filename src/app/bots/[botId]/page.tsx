...
import { Container, Typography, Button, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, TextField, Grid } from '@mui/material';
...
const BotDetails: React.FC = () => {
	...
	useEffect(() => {
		...
		socket.on('run_updated', (data) => {
			...
			if (data.bot_id === botId) {
				setRuns((prevRuns) =>
					prevRuns.map((run) => (run.id === data.id ? data : run))
				);
			}
		});
	...
	return (
		<Container>
			<Typography variant="h4" gutterBottom>Bot Details: {bot.name}</Typography>
			<Paper elevation={3} style={{ padding: '1em' }}>
				{isEditing ? (
					<Grid container spacing={2}>
						...
						<Grid item xs={12}>
							<TextField
								...
								onChange={(e) => setUpdatedBot({ ...updatedBot, script: e.target.value })}
							/>
						</Grid>
						<Grid item xs={12}>
							<TextField
								...
								onChange={(e) => setUpdatedBot({ ...updatedBot, schedule: e.target.value })}
							/>
						</Grid>
					</Grid>
				) : (
					<>
						...
						<Typography variant="body1">Schedule: {bot.schedule ? cronstrue.toString(bot.schedule) : 'Not scheduled'}</Typography>
					</>
				)}
			</Paper>
			...
			{isEditing ? (
				<>
					<Button variant="contained" color="primary" onClick={handleSaveBot} style={{ marginTop: '1em' }}>
						Save
					</Button>
					<Button variant="contained" onClick={handleCancelEdit} style={{ marginLeft: '10px', marginTop: '1em' }}>
						Cancel
					</Button>
				</>
			) : (
				<>
					<Button variant="contained" color="primary" onClick={handleRunBot} style={{ marginTop: '1em' }}>
						Run Bot
					</Button>
					<Button
						variant="contained"
			...
			<Typography variant="h5" gutterBottom style={{ marginTop: '1em' }}>Runs</Typography>
			<Paper elevation={3} style={{ padding: '1em' }}>
				<TableContainer>
					<Table>
						<TableHead>
						...
						<TableBody>
							{sortedRuns.map((run) => (
								<TableRow key={run.id} component={Link} href={`/runs/${run.id}`}>
									<TableCell>{run.id}</TableCell>
									<TableCell style={{ color: getStatusColor(run.status) }}>{run.status}</TableCell>
									<TableCell>{run.start_time ? moment(run.start_time).format('MMMM Do YYYY, h:mm:ss a') : 'N/A'}</TableCell>
									<TableCell>{run.end_time ? moment(run.end_time).format('MMMM Do YYYY, h:mm:ss a') : 'N/A'}</TableCell>
									<TableCell>{computeDuration(run.start_time, run.end_time)}</TableCell>
								</TableRow>
							))}
						</TableBody>
					</Table>
